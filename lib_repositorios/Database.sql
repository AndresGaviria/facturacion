...
(
	...
	CONSTRAINT [FK_Usuarios__Personas] FOREIGN KEY ([Persona]) REFERENCES [Personas] ([Id]) ON DELETE No Action ON UPDATE No Action,
)
GO
...
DECLARE @persona INT;
IF NOT EXISTS (SELECT 1 FROM [Usuarios] WHERE [Email] = 'admin@email.com')
BEGIN
	SET @persona = (SELECT [id] FROM [Personas] WHERE [Cedula] = '203');
	
	INSERT INTO [Usuarios] ([Email],[Contraseña],[Persona],[Activo])
	VALUES ('admin@email.com', 'D235dfg5871+', @persona, 1);
END 
